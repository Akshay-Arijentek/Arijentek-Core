...
def get_dashboard_data():
	...
	attendance_summary = {"Present": 0, "Absent": 0, "Half Day": 0}
	for row in summary:
...
def clock_out():
	...
	try:
		checkin = frappe.get_doc(
			{"doctype": "Employee Checkin", "employee": employee, "time": now_datetime(), "log_type": "OUT"}
		)
		checkin.insert(ignore_permissions=True)
		frappe.db.commit()
...
@frappe.whitelist()
def punch():
	"""Unified punch endpoint - automatically clocks in or out based on last status"""
	employee = get_current_employee()
	if not employee:
...
def get_leave_balance():
	...
	for leave_type in leave_types:
		try:
			...
			if balance > 0:
				balances.append({"leave_type": leave_type, "balance": balance})
		except:
			pass
